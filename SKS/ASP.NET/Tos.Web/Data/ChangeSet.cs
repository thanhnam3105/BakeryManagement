...
namespace Tos.Web.Data
{
    ...
    public static class ChangeSetExtensions {
        ...
        /// <param name="changeSet">changeSet</param>
        /// <param name="identity">ユーザー情報</param>
        public static void SetSeihoDataSaveInfo<T>(this ChangeSet<T> changeSet, IIdentity identity, bool isKeepTorokuInfor = false)
        {
            //TODO: cd_create/cd_updateが文字型の場合、下記ロジックを採用する
            //string userName = UserInfo.GetUserNameFromIdentity(identity);
            //TODO: cd_create/cd_updateが数値型の場合、下記ロジックを採用する
            ...
            {
                // TODO: プロジェクトで利用するテーブルの共通項目に応じて変更します
                if (!isKeepTorokuInfor || value.cd_toroku == null)
                {
                    value.cd_toroku = strName;
                    value.dt_toroku = date;
                }

                value.cd_koshin = strName;
                value.dt_koshin = date;
            }
            foreach (dynamic value in changeSet.Updated)
            {
