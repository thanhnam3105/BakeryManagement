 ...
 * MIT License
 * Copyright (c) 2016, Mayumi Nakamura
 * Modefied for jQuery3.2.1
 */
(function ($, undefined) {

    $.extend($.ui, { monthpicker: { version: "1.1.0" } });

    var PROP_NAME = "monthpicker",
	instActive;

    function monthpicker_getZindex(elem) {
        var position, value;
    ...
    $.extend(Monthpicker.prototype, {
        ...
        */
        ...
        _attachments: function (input, inst) {
            ...
            input.off("focus", this._showMonthpicker);
            ...
            if (showOn === "focus" || showOn === "both") { // pop-up month picker when in the marked field
                input.on("focus", this._showMonthpicker);
            }
        ...
        */
        _destroyMonthpicker: function (target) {
            ...
            if (nodeName === "input") {
				...
				off("focus", this._showMonthpicker).
...
//              inst.dpDiv.zIndex($(input).zIndex() + 1);  zIndex jQuery UIで廃止(1.12.1)
                inst.dpDiv.css("z-index", monthpicker_getZindex($(input)) + 1);
...
//                    inst.input.focus();
        ...
        _updateMonthpicker: function (inst) {
			...
			if ( activeCell.length > 0 ) {
				handleMouseover.apply( activeCell.get( 0 ) );
			}
        ...
        */
        ...
        _selectDay: function (id, month, year, day, td) {
            var inst,
			target = $(id);
        ...
        */
        _attachHandlers: function (inst) {
            ...
            inst.dpDiv.find("[data-handler]").map(function () {
                var handler = {
                    prev: function () {
                        $.monthpicker._adjustDate(id, -stepMonths, "M");
                    },
                    next: function () {
                        $.monthpicker._adjustDate(id, +stepMonths, "M");
                    },
                    ...
                    selectDay: function () {
                        $.monthpicker._selectDay(id, +this.getAttribute("data-month"), +this.getAttribute("data-year"), +this.getAttribute("data-day"), this);
                        return false;
                    },
        ...
        _generateHTML: function (inst) {
			...
			monthNames, monthNamesShort,
			defaultDate, html, dow, row, group, col, selectedDate,
			cornerClass, calender, numRows,
            ...
            for (row = 0; row < numMonths[0]; row++) {
                ...
                for (col = 0; col < numMonths[1]; col++) {
                    ...
                    "</div><table class='ui-datepicker-calendar' style='table-layout: fixed; margin: .4em 0;'><tbody>";

                    this.maxRows = numRows = 3;
                    ...
                    for (dRow = 0; dRow < numRows; dRow++) { // create month picker rows
                        ...
                        for (dow = 0; dow < 4; dow++) {
							...
							" data-handler='selectDay' data-event='click' data-month='" + printDate.getMonth() + "' data-year='" + printDate.getFullYear() + "' data-day='1'>" +
							"<a class='ui-state-default" +
							(printDate.getTime() === today.getTime() ? " ui-state-highlight" : "") +
							(printDate.getTime() === currentDate.getTime() ? " ui-state-active" : "") + // highlight selected day
                            "' href='#' style='padding: .7em .1em;'>" + monthNamesShort[printDate.getMonth()] + "</a></td>";
    ...
    */
    function bindHover(dpDiv) {
        ...
        }).on("mouseover", selector, handleMouseover);
    }

    function handleMouseover() {
        if (!$.monthpicker._isDisabledMonthpicker(instActive.inline ? instActive.dpDiv.parent()[0] : instActive.input[0])) {
    ...
    $.fn.monthpicker = function (options) {
        ...
        if (!$.monthpicker.initialized) {
            $(document).on("mousedown", $.monthpicker._checkExternalClick);
