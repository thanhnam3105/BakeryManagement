...
namespace Tos.Web.Controllers
{
    ...
    public class _105_ShishakuData_SanpuruSetsumeisho_Excel_Controller : ApiController
    {
        /// <summary>
        ...
        public HttpResponseMessage Get([FromUri] ParaDownLoad value)
        {
            ...
            using (ShohinKaihatsuEntities context = new ShohinKaihatsuEntities())
            {
                ...
                List<short> seqDownload = context.tr_shisaku.Where(m => m.cd_shain == value.cd_shain
                                                            && m.nen == value.nen
                                                            && m.no_oi == value.no_oi
                ...
                if (value.mode == 105)
                {
                    dirTemlates = templatepath + "\\" + Properties.Resources.SakuseiFomattoExcel + Properties.Resources.ExcelXlsmExtension;
                    filename = Properties.Resources.SakuseiFomattoExcelName;
                }
                else if (value.mode == 106)
                {
                    dirTemlates = templatepath + "\\" + Properties.Resources.SanpuruSetsumeishoExcel + Properties.Resources.ExcelXlsmExtension;
                    filename = Properties.Resources.SanpuruSetsumeishoExcelName;
                }
        ...
        /// </summary>
        /// <returns></returns>
        public bool isNotExitShisa(ParaDownLoad options)
        {
            using (ShohinKaihatsuEntities context = new ShohinKaihatsuEntities())
            {
        ...
        private MemoryStream EditExcelFileByClosedXML(ExcelFile excelFile, List<string> sheets, IEnumerable<sp_shohinkaihatsu_search_105_sanpuru_setsumeisho_Result> results, List<short> seqDownload)
        {
            var k = 0;
            var userInfo = Tos.Web.Data.UserInfo.CreateFromAuthorityMaster(this.User.Identity);
            ...
            foreach (var no_seq in seqDownload)
            {
                ...
                foreach (var itemDetail in data)
                {
                    ...
                    if (itemDetail.quantity != null) {
                        string quantity = itemDetail.quantity.ToString();
                        var listNuber = quantity.Split('.');
                        int keta_shosu = int.Parse(itemDetail.keta_shosu == null ? "0" : itemDetail.keta_shosu.ToString());
                        if (keta_shosu == 0 || listNuber.Length == 1)
                        {
                            itemDetail.quantity = decimal.Parse(listNuber[0]);
                        }
                        else
                        {
                            listNuber[1] = listNuber[1] + "0000";
                            itemDetail.quantity = decimal.Parse(listNuber[0] + "." + listNuber[1].Substring(0, keta_shosu));
                        }
                    }
                    excelFile.UpdateValue(sheetName1, "C" + no_row, itemDetail.quantity.ToString(), styleIndexC, true, false);
                    string budomari = "";
                    ...
                    excelFile.UpdateValue(sheetName1, "M" + no_row, itemDetail.memo, 0, true, false);
                ...
                excelFile.UpdateValue(sheetName1, "B155", "合計重量(g)", 0, true, false);
                ...
                excelFile.UpdateValue(sheetName2, "S64", itemHeader.nm_team, 0, true, false);
                ...
                excelFile.UpdateValue(sheetName2, "H67", itemHeader.nm_ikatuhyouzi, 0, true, false);
                excelFile.UpdateValue(sheetName2, "H68", itemHeader.nm_shain_nen_oi, 0, true, false);
        ...
        public List<sp_shohinkaihatsu_search_105_sanpuru_setsumeisho_Result> GetData105(ParaDownLoad value, ShohinKaihatsuEntities context)
        {
            List<sp_shohinkaihatsu_search_105_sanpuru_setsumeisho_Result> results = context.sp_shohinkaihatsu_search_105_sanpuru_setsumeisho(value.cd_shain, value.nen, value.no_oi).ToList();
