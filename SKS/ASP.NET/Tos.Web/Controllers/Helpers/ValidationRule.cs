    ...
    public class ValidationRule
    {
        ...
        public ValidationRule(Dictionary<string, object> ruleTypes, object target)
        {
            foreach (var ruleType in ruleTypes)
            {
                ...
                else if (ruleType.Key == ValidationRuleTypes.Maxmojisu)
                {
                    string value = target.GetType() == typeof(System.DBNull) ? target.ToString() : (string)target;
                    int length = int.Parse(ruleType.Value.ToString());
                    rules.Add(() => value.Maxmojisu(length));
                }
                //add 20230309: TosVN 22100
                //start
                else if (ruleType.Key == ValidationRuleTypes.MaxLengthCustom)
                {
                    string value = target.GetType() == typeof(System.DBNull) ? target.ToString() : (string)target;
                    int length = int.Parse(ruleType.Value.ToString());
                    rules.Add(() => value.MaxByteLengthCustom(length));
                }
                else if (ruleType.Key == ValidationRuleTypes.RangeCustom)
                {
                    string value = target.GetType() == typeof(System.DBNull) ? target.ToString() : (string)target;
                    TextFieldSetting.Range range = (TextFieldSetting.Range)ruleType.Value;
                    rules.Add(() => value.RangeCustom(range.Min, range.Max));
                }
                else if (ruleType.Key == ValidationRuleTypes.PointLengthCustom)
                {
                    string value = target.GetType() == typeof(System.DBNull) ? target.ToString() : (string)target;
                    TextFieldSetting.PointLength pointcheck = (TextFieldSetting.PointLength)ruleType.Value;
                    rules.Add(() => value.PointLengthCustom(pointcheck.BeforePoint, pointcheck.AfterPoint, pointcheck.AllowNegativeNumber));
                }
                //end
