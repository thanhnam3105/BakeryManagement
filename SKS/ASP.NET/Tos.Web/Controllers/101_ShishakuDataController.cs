...
namespace Tos.Web.Controllers
{
    ...
    public class ShishakuDataController : ApiController
    {
        ...
        public LoadMasterDataRespon GetMasterData([FromUri] paramSearch paraSearch)
        {
            using (ShohinKaihatsuEntities context = new ShohinKaihatsuEntities())
            {
                LoadMasterDataRespon Results = new LoadMasterDataRespon();
                Results.ShubetsuBango = context.ma_literal.Where(x => x.cd_category == paraSearch.shubetsu_bango).OrderBy(x => x.no_sort).ToList();
                Results.KoteiPatan = context.ma_literal.Where(x => x.cd_category == paraSearch.kotei_patan).OrderBy(x => x.no_sort).ToList();
                Results.CyuuiBango = context.tr_cyuui.Where(x => x.cd_shain == paraSearch.cd_shain && x.nen == paraSearch.nen && x.no_oi == paraSearch.no_oi).OrderBy(x => x.no_chui).ToList();
                Results.no_chui_max = Results.CyuuiBango.OrderByDescending(x => x.no_chui).Select(x => x.no_chui).FirstOrDefault();
                Results.KoteiZokusei = context.ma_literal.Where(x => x.cd_category == paraSearch.Kotei_zokusei).OrderBy(x => x.no_sort).ToList();
        ...
        public HttpResponseMessage Get([FromUri] paramSearch paraSearch)
        { 
            //原価試算依頼ある。
            int checkIri = 0;
            using (ShohinKaihatsuEntities contextnew = new ShohinKaihatsuEntities())
            {
                checkIri = contextnew.tr_shisaku.Where(x => x.cd_shain == paraSearch.cd_shain && x.nen == paraSearch.nen && x.no_oi == paraSearch.no_oi && x.flg_shisanIrai == flg_shisanIrai).Take(1).ToList().Count();
            }

            using (ShohinKaihatsuEntities context = new ShohinKaihatsuEntities())
            {
                //■試作品テーブル 登録
                var shisakuhin = (from shisa in (context.tr_shisakuhin.Where(x => x.cd_shain == paraSearch.cd_shain && x.nen == paraSearch.nen && x.no_oi == paraSearch.no_oi))
                                  from sampl in context.tr_shisaku.Where(x => x.cd_shain == shisa.cd_shain && x.nen == shisa.nen && x.no_oi == shisa.no_oi && x.seq_shisaku == shisa.seq_shisaku).DefaultIfEmpty()
                                  from user in context.ma_user_togo.Where(x => x.id_user == shisa.cd_eigyo).DefaultIfEmpty()
                                  select new
                                  {
                ...
                var results = (from shisa in shisakuhin
                               from koshin in context.ma_user_togo.Where(x => x.id_user == shisa.id_koshin).DefaultIfEmpty()
                               from toroku in context.ma_user_togo.Where(x => x.id_user == shisa.id_toroku).DefaultIfEmpty()
                ...
                return Request.CreateResponse<shisakuhinResult>(HttpStatusCode.OK, results);
                //return ;
            }
        }
        ...
        /// <param name="paraSearch"></param>
        /// <returns></returns>
        public HttpResponseMessage Post_haita([FromBody] paramSearch paraSearch)
        {
            if (paraSearch == null)
            {
            ...
            {
                inforUsingDataHaita results = new inforUsingDataHaita();
                try
                {
                    // 2020-03-13 : START : Request #16147 複数人で画面起動時の制御
                    //TODO: DbContext のメソッドとしてストアドプロシージャを呼び出します。※ストアドのアウトプットパラメータとしてcountが返却される前提です
                    var Items = contextHaita.sp_shohinkaihatsu_haita_check_101(paraSearch.cd_shain, paraSearch.nen, paraSearch.no_oi, paraSearch.EmployeeCD, paraSearch.isOpen).ToList();
                    if (Items.Count() < 1)
                    {
                        return Request.CreateResponse<string>(HttpStatusCode.BadRequest, "AP0007");
                    }
                    ...
                    ////file shisa
                    //var item = (from x in contextHaita.tr_shisakuhin
                    ...
                    //ChangeSet<tr_shisakuhin> changeSetHaita = new ChangeSet<tr_shisakuhin>();
        ...
        public HttpResponseMessage Post_GenryoDelivery([FromBody] List<wk_shisaku_genryo_delivery> value)
        {
            int maxKey = 0;
            using (ShohinKaihatsuEntities context = new ShohinKaihatsuEntities())
            {
                ChangeSet<wk_shisaku_genryo_delivery> deliveryChangeset = new ChangeSet<wk_shisaku_genryo_delivery>();

                //START : 一番高いセッションIDを貰います
                maxKey = context.wk_shisaku_genryo_delivery.OrderByDescending(x => x.id_session).Select(x => x.id_session).FirstOrDefault() + 1;
        ...
        public HttpResponseMessage Post([FromBody]ShikakuDataChangeSet value)
        {
            ...
            tr_shisakuhin ReturnValue = new tr_shisakuhin();
            using (ShohinKaihatsuEntities context = new ShohinKaihatsuEntities())
            {
                ...
                using (DbTransaction transaction = connection.BeginTransaction())
                {
                    ...
                    {
                        ChangeSet<ma_shisaku_saiban> shisaku_saiban = new ChangeSet<ma_shisaku_saiban>();
                        tr_gate_header gate_header = new tr_gate_header();
                        value.tr_gate_header.Created.Add(gate_header);
                        ...
                        {
                            return Request.CreateResponse<string>(HttpStatusCode.BadRequest, "ma_shisaku_saiban");
                        }
                    }

                    //試作品テーブル : キー項目の重複チェックを行います。
                    InvalidationSet<tr_shisakuhin> shisakuhinInvalidations = shisakuhinIsAlreadyExists(value.tr_shisakuhin, ref ReturnValue);
                    if (shisakuhinInvalidations.Count > 0)
                    {
                        return Request.CreateResponse<InvalidationSet<tr_shisakuhin>>(HttpStatusCode.BadRequest, shisakuhinInvalidations);
                    }
                    SaveShisakuhin(context, value.tr_shisakuhin);

                    //配合トラン : キー項目の重複チェックを行います。
                    InvalidationSet<tr_haigo> haigoInvalidations = haigoIsAlreadyExists(value.tr_haigo, ref ReturnValue);
                    if (haigoInvalidations.Count > 0)
                    {
                        return Request.CreateResponse<InvalidationSet<tr_haigo>>(HttpStatusCode.BadRequest, haigoInvalidations);
                    }
                    SaveHaigo(context, value.tr_haigo);

                    //試作テーブル : キー項目の重複チェックを行います。
                    InvalidationSet<tr_shisaku> shisakuInvalidations = shisakuIsAlreadyExists(value.tr_shisaku, ref ReturnValue);
                    if (shisakuInvalidations.Count > 0)
                    {
                        return Request.CreateResponse<InvalidationSet<tr_shisaku>>(HttpStatusCode.BadRequest, shisakuInvalidations);
                    }
                    SaveShisaku(context, value.tr_shisaku);

                    //試作リストテーブル : キー項目の重複チェックを行います。
                    InvalidationSet<tr_shisaku_list> shisakuListInvalidations = shisakuListIsAlreadyExists(value.tr_shisaku_list, ref ReturnValue);
                    if (shisakuListInvalidations.Count > 0)
                    {
                        return Request.CreateResponse<InvalidationSet<tr_shisaku_list>>(HttpStatusCode.BadRequest, shisakuListInvalidations);
                    }
                    SaveShisakuList(context, value.tr_shisaku_list);

                    //原価原料トラン : キー項目の重複チェックを行います。
                    InvalidationSet<tr_genryo> genryoInvalidations = genryoIsAlreadyExists(value.tr_genryo, ref ReturnValue);
                    if (genryoInvalidations.Count > 0)
                    {
                        return Request.CreateResponse<InvalidationSet<tr_genryo>>(HttpStatusCode.BadRequest, genryoInvalidations);
                    }

                    SaveGenryo(context, value.tr_genryo);

                    //製造工程テーブル : キー項目の重複チェックを行います。
                    InvalidationSet<tr_cyuui> cyuuiInvalidations = cyuuiIsAlreadyExists(value.tr_cyuui, ref ReturnValue);
                    if (cyuuiInvalidations.Count > 0)
                    {
                        return Request.CreateResponse<InvalidationSet<tr_cyuui>>(HttpStatusCode.BadRequest, cyuuiInvalidations);
                    }

                    SaveCyuui(context, value.tr_cyuui);
            ...
            return Request.CreateResponse<tr_shisakuhin>(HttpStatusCode.OK, ReturnValue);
        }
        ...
        public void CopyForSeiho(ShohinKaihatsuEntities context, ShikakuDataChangeSet value, settingDefault setting, decimal cd_haigo_max)
        {
            decimal cd_login = decimal.Parse(UserInfo.GetUserNameFromIdentity(this.User.Identity));
            DateTime sysTime = DateTime.Now;

            if (value.tr_shisakuhin.Updated.Count() <= 0)
            {
                value.tr_shisakuhin.Updated.Add(context.tr_shisakuhin.Where(x => x.cd_shain == value.cd_shain && x.nen == value.nen && x.no_oi == value.no_oi).FirstOrDefault());
            }

            value.tr_shisakuhin.Updated[0].seq_shisaku = value.kopi.seq_shisaku;
            value.tr_shisakuhin.Updated[0].id_koshin = cd_login;
            value.tr_shisakuhin.Updated[0].dt_koshin = sysTime;

            //save shisakuhin
            value.tr_shisakuhin.AttachTo(context);
            context.SaveChanges();

            var shisakuhin = value.tr_shisakuhin.Updated[0];
            ...
            var shisaku = context.tr_shisaku.Where(x => x.cd_shain == value.cd_shain && x.nen == value.nen && x.no_oi == value.no_oi && x.seq_shisaku == value.kopi.seq_shisaku).FirstOrDefault();
            ...
            value.tr_shisaku.Updated.Add(shisaku);
            ...
            //■配合明細 登録
            var haigo = context.tr_haigo.Where(x => x.cd_shain == value.cd_shain && x.nen == value.nen && x.no_oi == value.no_oi).OrderBy(x => x.sort_kotei).ThenBy(x => x.sort_genryo);
            var genryo_seiho = context.ma_genryo_seiho.Where(x => x.cd_kaisha == shisakuhin.cd_kaisha && x.cd_kojyo == shisakuhin.cd_kojo);
            var shisaku_list = context.tr_shisaku_list.Where(x => x.cd_shain == value.cd_shain && x.nen == value.nen && x.no_oi == value.no_oi && x.seq_shisaku == value.kopi.seq_shisaku);
            ...
            foreach (var item in haigo)
            {
                ...
                //配合重量
                var qtyHaigo = qty_haigo(shisaku_list, item, cd_hin, setting);
        ...
        public double? Budomari(ma_genryo_seiho genryo_sh, decimal cd_hin, settingDefault setting)
        {
            //品コード アンマッチの場合
            if (genryo_sh == null)
            {
        ...
        /// <returns>double?</returns>
        public double? qty_haigo(IEnumerable<tr_shisaku_list> shisaku_list, tr_haigo haigo, decimal cd_hin, settingDefault setting)
        {
            //原料でコード＝999999の場合
            if (cd_hin == setting.cd_command)
            {
        ...
        public decimal Genryocode(string cd_genryo, settingDefault setting)
        {
        ...
        public HttpResponseMessage iraiData(ShohinKaihatsuEntities context, tr_shisakuhin ReturnValue, List<iraiSelected> iraiSelected, string daihyogaisha, short cd_kaisha, int? cd_busho, string cd_nisugata, settingDefault setting)
        {
            ...
            var shisakuhin = context.tr_shisakuhin.Where(x => x.cd_shain == ReturnValue.cd_shain && x.nen == ReturnValue.nen && x.no_oi == ReturnValue.no_oi).FirstOrDefault();
            ...
            var irai = (from c in iraiSelected.Where(x => x.flg_disabled == false)
                        join p in context.tr_shisaku.Where(x => x.cd_shain == shisakuhin.cd_shain && x.nen == shisakuhin.nen && x.no_oi == shisakuhin.no_oi)
            ...
            if (irai.Count() > 0)
            {
                ...
                var shisakuhin_check = (
                    ...
                    from shisa in context.tr_shisakuhin.Where(x => x.cd_shain == shisan.cd_shain && x.nen == shisan.nen && x.no_oi == shisan.no_oi).DefaultIfEmpty()
                    select new
                    {
                        cd_shain = shisa.cd_shain,
                        nen = shisa.nen,
                ...
                //６）試算配合テーブルへデータ登録
                //  試算配合テーブルに検索
                var shisanHaigo = (from haigo in context.tr_haigo.Where(x => x.cd_shain == shisakuhin.cd_shain && x.nen == shisakuhin.nen && x.no_oi == shisakuhin.no_oi)
            ...
            if (cancelIrai.Count() > 0)
            {
                ...
                //tr_shisaku 更新
                ChangeSet<tr_shisaku> shisaku_up = new ChangeSet<tr_shisaku>();
                var shisakuUp = (from cancel in cancelIrai
                                 join shisaku in context.tr_shisaku.Where(x => x.cd_shain == shisakuhin.cd_shain && x.nen == shisakuhin.nen && x.no_oi == shisakuhin.no_oi)
                                 on cancel.seq_shisaku equals shisaku.seq_shisaku
                                 select shisaku);
        ...
        private void ShisanHaigoKoshin(ShohinKaihatsuEntities context, tr_shisakuhin shisakuhin, List<tr_shisan_shisakuhin> shisakuhinList, ref ChangeSet<tr_shisan_shisakuhin> shisakuhinKoshin, ref ChangeSet<tr_shisan_haigo> haigoKoshin, decimal no_eda, DateTime sysTime, decimal? cd_login, settingDefault setting)
        {
            //洗い替え原料データ検索
        ...
        /// <returns></returns>
        private InvalidationSet<tr_shisakuhin> shisakuhinIsAlreadyExists(ChangeSet<tr_shisakuhin> value, ref tr_shisakuhin ReturnValue)
        {
            InvalidationSet<tr_shisakuhin> result = new InvalidationSet<tr_shisakuhin>();
            ...
            using (ShohinKaihatsuEntities context = new ShohinKaihatsuEntities())
            {
                foreach (var item in value.Created)
                {
                    ...
                    var isDatabaseExists = (context.tr_shisakuhin.Find(item.cd_shain, item.nen, item.no_oi) != null);
                    ...
                    if (isDepulicate)
                    {
                        result.Add(new Invalidation<tr_shisakuhin>(Properties.Resources.ValidationKey, item, "tr_shisakuhin"));
                    }
                }
        ...
        /// <param name="value">target 情報の変更セット</param>
        /// <returns></returns>
        private InvalidationSet<tr_haigo> haigoIsAlreadyExists(ChangeSet<tr_haigo> value, ref tr_shisakuhin ReturnValue)
        {
            InvalidationSet<tr_haigo> result = new InvalidationSet<tr_haigo>();
            ...
            using (ShohinKaihatsuEntities context = new ShohinKaihatsuEntities())
            {
                ...
                foreach (var item in value.Created)
                {
                    ...
                    var isDatabaseExists = (context.tr_haigo.Find(item.cd_shain, item.nen, item.no_oi, item.cd_kotei, item.seq_kotei) != null);
                    ...
                    if (isDepulicate)
                    {
                        result.Add(new Invalidation<tr_haigo>(Properties.Resources.ValidationKey, item, "tr_haigo"));
                    }
                    if (isFirst)
                    {
                        isFirst = false;
        ...
        /// <param name="value">target 情報の変更セット</param>
        /// <returns></returns>
        private InvalidationSet<tr_shisaku> shisakuIsAlreadyExists(ChangeSet<tr_shisaku> value, ref tr_shisakuhin ReturnValue)
        {
            InvalidationSet<tr_shisaku> result = new InvalidationSet<tr_shisaku>();
            ...
            using (ShohinKaihatsuEntities context = new ShohinKaihatsuEntities())
            {
                ...
                foreach (var item in value.Created)
                {
                    ...
                    var isDatabaseExists = (context.tr_shisaku.Find(item.cd_shain, item.nen, item.no_oi, item.seq_shisaku) != null);
                    ...
                    if (isDepulicate)
                    {
                        result.Add(new Invalidation<tr_shisaku>(Properties.Resources.ValidationKey, item, "tr_shisaku"));
                    }
        ...
        /// <param name="value">target 情報の変更セット</param>
        /// <returns></returns>
        private InvalidationSet<tr_shisaku_list> shisakuListIsAlreadyExists(ChangeSet<tr_shisaku_list> value, ref tr_shisakuhin ReturnValue)
        {
            InvalidationSet<tr_shisaku_list> result = new InvalidationSet<tr_shisaku_list>();
            ...
            using (ShohinKaihatsuEntities context = new ShohinKaihatsuEntities())
            {
                ...
                foreach (var item in value.Created)
                {
                    ...
                    var isDatabaseExists = (context.tr_shisaku_list.Find(item.cd_shain, item.nen, item.no_oi, item.seq_shisaku, item.cd_kotei, item.seq_kotei) != null);
                    ...
                    if (isDepulicate)
                    {
                        result.Add(new Invalidation<tr_shisaku_list>(Properties.Resources.ValidationKey, item, "tr_shisaku_list"));
                    }
        ...
        /// <param name="value">target 情報の変更セット</param>
        /// <returns></returns>
        private InvalidationSet<tr_genryo> genryoIsAlreadyExists(ChangeSet<tr_genryo> value, ref tr_shisakuhin ReturnValue)
        {
            InvalidationSet<tr_genryo> result = new InvalidationSet<tr_genryo>();
            ...
            using (ShohinKaihatsuEntities context = new ShohinKaihatsuEntities())
            {
                ...
                foreach (var item in value.Created)
                {
                    ...
                    var createdCount = value.Created.Count(target => target.cd_shain == item.cd_shain && target.nen == item.nen && target.no_oi == item.no_oi && target.seq_shisaku == item.seq_shisaku);
                    var isDeleted = value.Deleted.Exists(target => target.cd_shain == item.cd_shain && target.nen == item.nen && target.no_oi == item.no_oi && target.seq_shisaku == item.seq_shisaku);
                    var isDatabaseExists = (context.tr_genryo.Find(item.cd_shain, item.nen, item.no_oi, item.seq_shisaku) != null);
                    ...
                    if (isDepulicate)
                    {
                        result.Add(new Invalidation<tr_genryo>(Properties.Resources.ValidationKey, item, "tr_genryo"));
                    }
        ...
        /// <param name="value">target 情報の変更セット</param>
        /// <returns></returns>
        private InvalidationSet<tr_cyuui> cyuuiIsAlreadyExists(ChangeSet<tr_cyuui> value, ref tr_shisakuhin ReturnValue)
        {
            InvalidationSet<tr_cyuui> result = new InvalidationSet<tr_cyuui>();
            ...
            using (ShohinKaihatsuEntities context = new ShohinKaihatsuEntities())
            {
                ...
                foreach (var item in value.Created)
                {
                    ...
                    var isDatabaseExists = (context.tr_cyuui.Find(item.cd_shain, item.nen, item.no_oi, item.no_chui) != null);
                    ...
                    if (isDepulicate)
                    {
                        result.Add(new Invalidation<tr_cyuui>(Properties.Resources.ValidationKey, item, "tr_cyuui"));
                    }
        ...
        /// <param name="context"></param>
        /// <param name="value"></param>
        public void updateKey(ShohinKaihatsuEntities context, ref ShikakuDataChangeSet value, ref ChangeSet<ma_shisaku_saiban> shisaku_saiban, ref tr_shisakuhin ReturnValue)
        {
            #region get infor key tr_shisakuhin
            ...
            var no_oi_max = context.ma_shisaku_saiban.Where(x => x.cd_shain == cd_shain && x.nen == nen).FirstOrDefault();

            if (no_oi_max == null)
            {
                shisaku_saiban.Created.Add(new ma_shisaku_saiban
                {
                    cd_shain = cd_shain,
                    nen = nen,
                    no_oi = no_oi
            ...
            {
                no_oi = no_oi_max.no_oi + no_oi;
                no_oi_max.no_oi = no_oi;
                shisaku_saiban.Updated.Add(no_oi_max);
            }

            value.tr_shisakuhin.Created[0].cd_shain = cd_shain;
            value.tr_shisakuhin.Created[0].nen = nen;
            value.tr_shisakuhin.Created[0].no_oi = no_oi;

            value.tr_gate_header.Created[0].cd_shain = cd_shain;
            value.tr_gate_header.Created[0].nen = nen;
            value.tr_gate_header.Created[0].no_oi = no_oi;
            value.tr_gate_header.Created[0].flg_shonin_tanto_plan = false;
            value.tr_gate_header.Created[0].flg_shonin_reader_plan = false;
            value.tr_gate_header.Created[0].flg_shonin_tanto_confirm = false;
            value.tr_gate_header.Created[0].flg_shonin_reader_confirm = false;
            ...
            {
                value.tr_shisakuhin.Created[0].ts = null;
            }

            #endregion get infor key tr_shisakuhin

            #region update key haigo

            foreach (var item in value.tr_haigo.Created)
            {
            ...
            #region update key tr_shisaku

            foreach (var item in value.tr_shisaku.Created)
            {
                item.cd_shain = cd_shain;
                item.nen = nen;
                item.no_oi = no_oi;
            ...
            #endregion update key tr_shisaku

            #region update key tr_shisaku_list

            foreach (var item in value.tr_shisaku_list.Created)
            {
                item.cd_shain = cd_shain;
                item.nen = nen;
                item.no_oi = no_oi;
            ...
            #endregion update key tr_shisaku_list

            #region update key tr_genryo

            foreach (var item in value.tr_genryo.Created)
            {
                item.cd_shain = cd_shain;
                item.nen = nen;
                item.no_oi = no_oi;
            ...
            #endregion update key tr_genryo

            #region update key tr_cyuui

            foreach (var item in value.tr_cyuui.Created)
            {
                item.cd_shain = cd_shain;
                item.nen = nen;
                item.no_oi = no_oi;
        ...
        private void SaveGenryo(ShohinKaihatsuEntities context, ChangeSet<tr_genryo> header)
        {
        ...
        /// <param name="context"></param>
        /// <param name="header"></param>
        private void SaveCyuui(ShohinKaihatsuEntities context, ChangeSet<tr_cyuui> header)
        {
            // TODO: 登録日、ユーザー、更新日、ユーザーをセットします。
            header.SetDataSaveInfo(this.User.Identity);
            header.AttachTo(context);
        ...
        /// <summary>
        /// Save tr_shisaku_list
        /// </summary>
        /// <param name="context"></param>
        /// <param name="header"></param>
        private void SaveShisakuList(ShohinKaihatsuEntities context, ChangeSet<tr_shisaku_list> header)
        {
            // TODO: 登録日、ユーザー、更新日、ユーザーをセットします。
            header.SetDataSaveInfo(this.User.Identity);
            header.AttachTo(context);
        ...
        /// <param name="context"></param>
        /// <param name="header"></param>
        private void SaveShisaku(ShohinKaihatsuEntities context, ChangeSet<tr_shisaku> header)
        {
            // TODO: 登録日、ユーザー、更新日、ユーザーをセットします。
            header.SetDataSaveInfo(this.User.Identity);
            header.AttachTo(context);
        ...
        /// <param name="context"></param>
        /// <param name="header"></param>
        private void SaveHaigo(ShohinKaihatsuEntities context, ChangeSet<tr_haigo> header)
        {
            // TODO: 登録日、ユーザー、更新日、ユーザーをセットします。
            header.SetDataSaveInfo(this.User.Identity);
            header.AttachTo(context);
        ...
        private void SaveShisakuhin(ShohinKaihatsuEntities context, ChangeSet<tr_shisakuhin> header)
        {
            // TODO: 登録日、ユーザー、更新日、ユーザーをセットします。
            header.SetDataSaveInfo(this.User.Identity);
            header.AttachTo(context);
        ...
        /// <summary>
        /// save ma_shisaku_saiban
        /// </summary>
        /// <param name="context">DbContext</param>
        /// <param name="header">hedaer情報</param>
        /// <returns>headerの新規キー項目</returns>
        private void SaveShisakuSaiban(ShohinKaihatsuEntities context, ChangeSet<ma_shisaku_saiban> header)
        {
            header.SetDataSaveInfo(this.User.Identity);
            header.AttachTo(context);
            context.SaveChanges();
    ...
    /// </summary>
    public class LoadMasterDataRespon
    {
        public LoadMasterDataRespon()
        {
            this.ShubetsuBango = new List<ma_literal>();
            this.KoteiPatan = new List<ma_literal>();
            this.CyuuiBango = new List<tr_cyuui>();
        ...
        public List<tr_cyuui> CyuuiBango { get; set; }
    ...
    {
        public ShikakuDataChangeSet()
        {
            this.tr_gate_header = new ChangeSet<tr_gate_header>();
        }
        ...
        public ChangeSet<tr_shisakuhin> tr_shisakuhin { get; set; }
        public ChangeSet<tr_haigo> tr_haigo { get; set; }
        public ChangeSet<tr_shisaku> tr_shisaku { get; set; }
        public ChangeSet<tr_shisaku_list> tr_shisaku_list { get; set; }
        public ChangeSet<tr_cyuui> tr_cyuui { get; set; }
        public ChangeSet<tr_genryo> tr_genryo { get; set; }
        public ChangeSet<tr_gate_header> tr_gate_header { get; set; }
    ...
    public class paramSearch
    {
        ...
        public string kotei_patan { get; set; }
        public string Kotei_zokusei { get; set; }
    ...
    /// result shisakuhin
    /// </summary>
    public class shisakuhinResult : tr_shisakuhin
    {
    ...
    /// Request #16147 : 使用中（他の人が開いている）の場合、初期表示時に「他の方が開いているため、参照モードで開きます」のメッセージを通知し、参照モードで開く。
    /// </summary>
    public class inforUsingDataHaita
    {
