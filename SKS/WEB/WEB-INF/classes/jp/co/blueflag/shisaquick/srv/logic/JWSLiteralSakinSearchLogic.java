...
/**
 * 
 * 【S3-67 : SA720】 JWSリテラル検索（殺菌方法）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralSakinSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralSakinSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_sakinhoho";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（殺菌方法）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
