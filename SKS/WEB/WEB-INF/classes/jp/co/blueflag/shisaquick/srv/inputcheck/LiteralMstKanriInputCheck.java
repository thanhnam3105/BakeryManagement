 ...
 * @since 2009/04/07
 */
public class LiteralMstKanriInputCheck extends InputCheck {

	/**
	 * コンストラクタ : 各リテラルマスタ更新処理インプットチェックコンストラクタ
	 */
	public LiteralMstKanriInputCheck() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			...
			// 機能ID：SA330のインプットチェック（更新内容チェック）を行う。
	...
	private void kanriValueCheck(RequestData checkData) throws ExceptionSystem,
		...
		try {
			if (checkData.GetValueStr("SA330", 0, 0, "kbn_shori").equals("1")) {
				// 処理区分＝登録の場合、登録値チェックを行う。
				insertValueCheck(checkData);
			}else if (checkData.GetValueStr("SA330", 0, 0, "kbn_shori").equals("2")) {
				// 処理区分＝更新の場合、更新値チェックを行う。
				updateValueCheck(checkData);
			}else if (checkData.GetValueStr("SA330", 0, 0, "kbn_shori").equals("3")) {
	...
	private void insertValueCheck(RequestData checkData) throws ExceptionSystem,
		...
		try {
			...
			super.hissuCodeCheck(checkData.GetValueStr("SA330", 0, 0, "cd_category"), "カテゴリ");
			// リテラル名の必須入力チェック（スーパークラスの必須チェック）を行う。
			// ※引数1：リテラル名	 メッセージパラメータ："リテラル名"　メッセージコード：E000200
			super.hissuInputCheck(checkData.GetValueStr("SA330", 0, 0, "nm_literal"), "リテラル名");
			// リテラル名の入力桁数チェック（スーパークラスの入力桁数チェック）を行う。
			// ※引数1：リテラル名、引数2：60		メッセージパラメータ："リテラル名"　メッセージコード：E000201　引数4：60
			super.sizeCheckLen(checkData.GetValueStr("SA330", 0, 0, "nm_literal"), "リテラル名", 60);
			// 表示順の必須入力チェックを行う。
			// ※引数1：表示順 		メッセージパラメータ："表示順"　メッセージコード：E000200
			super.hissuInputCheck(checkData.GetValueStr("SA330", 0, 0, "no_sort"), "表示順");
			if (!checkData.GetValueStr("SA330", 0, 0, "biko").toString().equals("")) {
				// 備考の入力桁数チェック（スーパークラスの入力桁数チェック）を行う。
				// ※引数1：備考、引数2：60 		メッセージパラメータ："備考"　メッセージコード：E000200
				super.sizeCheckLen(checkData.GetValueStr("SA330", 0, 0, "biko"), "備考", 60);
			}
			// 編集可否フラグの必須入力チェックを行う。
			// ※引数1：編集可否フラグ 	メッセージパラメータ："編集可否"　メッセージコード：E000207
			super.hissuCodeCheck(checkData.GetValueStr("SA330", 0, 0, "flg_edit"), "編集可否");
	...
	private void updateValueCheck(RequestData checkData) throws ExceptionSystem,
		...
		try {
			...
			super.hissuCodeCheck(checkData.GetValueStr("SA330", 0, 0, "cd_literal"), "リテラル");
			// リテラル名の必須入力チェック（スーパークラスの必須チェック）を行う。
			// ※引数1：リテラル名 			メッセージパラメータ："リテラル名"　メッセージコード：E000200
			super.hissuInputCheck(checkData.GetValueStr("SA330", 0, 0, "nm_literal"), "リテラル名");
			// リテラル名の入力桁数チェック（スーパークラスの入力桁数チェック）を行う。
