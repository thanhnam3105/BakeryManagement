...
/**
 * 
 * 【S3-64 : SA690】 JWSリテラル検索（担当営業）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralTantoSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralTantoSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_tantoeigyo";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（担当営業）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
