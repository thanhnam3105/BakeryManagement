...
/**
 * 
 * 【S3-58 : SA630】 JWSリテラル検索（ユーザ）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralUserSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralUserSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_yuza";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（ユーザ）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
