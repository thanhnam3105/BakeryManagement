 ...
 * @since  2009/03/29
 */
public class TokuchouGenryouSearchLogic extends LogicBase{
	
	/**
	 * コンボ用：特徴原料情報取得検索DB処理用コンストラクタ 
	 * : インスタンス生成
	 */
	public TokuchouGenryouSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			...
			//レスポンスデータの形成
			storageTokuchouGenryouCmb(lstRecset, resKind.getTableItem(strTableNm));
	...
	/**
	 * コンボ用：特徴原料パラメーター格納
	 *  : 特徴原料コンボボックス情報をレスポンスデータへ格納する。
	 * @param lstTokuchouGenryouCmb : 検索結果情報リスト
	 * @throws ExceptionWaning 
	 * @throws ExceptionUser 
	 * @throws ExceptionSystem 
	 */
	private void storageTokuchouGenryouCmb(List<?> lstTokuchouGenryouCmb, RequestResponsTableBean resTable) 
	throws ExceptionSystem, ExceptionUser, ExceptionWaning{

		try {
			
			for (int i = 0; i < lstTokuchouGenryouCmb.size(); i++) {
				...
				Object[] items = (Object[]) lstTokuchouGenryouCmb.get(i);
			...
			if (lstTokuchouGenryouCmb.size() == 0) {
