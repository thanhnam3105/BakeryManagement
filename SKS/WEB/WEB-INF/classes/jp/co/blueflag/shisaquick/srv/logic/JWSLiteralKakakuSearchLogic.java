...
/**
 * 
 * 【S3-61 : SA660】 JWSリテラル検索（価格帯）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralKakakuSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralKakakuSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_kakakutai";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（価格帯）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
