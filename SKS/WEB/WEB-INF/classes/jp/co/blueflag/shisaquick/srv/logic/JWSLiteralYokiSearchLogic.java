...
/**
 * 
 * 【S3-68 : SA730】 JWSリテラル検索（容器包材）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralYokiSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralYokiSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_yoki";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（容器包材）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
