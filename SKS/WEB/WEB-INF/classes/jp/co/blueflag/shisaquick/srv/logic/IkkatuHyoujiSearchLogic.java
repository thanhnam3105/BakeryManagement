 ...
 * @since  2009/03/29
 */
public class IkkatuHyoujiSearchLogic extends LogicBase{
	
	/**
	 * コンボ用：一括表示名称情報検索DB処理用コンストラクタ 
	 * : インスタンス生成
	 */
	public IkkatuHyoujiSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	/**
	 * コンボ用：一括表示名称情報取得SQL作成
	 *  : 一括表示名称コンボボックス情報を取得するSQLを作成。
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			...
			//レスポンスデータの形成
			storageIkkatuHyoujCmb(lstRecset, resKind.getTableItem(0));
	...
	/**
	 * コンボ用：一括表示名称パラメーター格納
	 *  : 一括表示名称コンボボックス情報をレスポンスデータへ格納する。
	 * @param lstIkkatuHyoujiCmb : 検索結果情報リスト
	 * @throws ExceptionWaning 
	 * @throws ExceptionUser 
	 * @throws ExceptionSystem 
	 */
	private void storageIkkatuHyoujCmb(List<?> lstIkkatuHyoujiCmb, RequestResponsTableBean resTable) 
		throws ExceptionSystem, ExceptionUser, ExceptionWaning{

		try {
			
			for (int i = 0; i < lstIkkatuHyoujiCmb.size(); i++) {
				...
				Object[] items = (Object[]) lstIkkatuHyoujiCmb.get(i);
