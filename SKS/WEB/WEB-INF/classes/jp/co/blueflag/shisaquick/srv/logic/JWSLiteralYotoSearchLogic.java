...
/**
 * 
 * 【S3-60 : SA650】 JWSリテラル検索（用途）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralYotoSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralYotoSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_yoto";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（用途）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
