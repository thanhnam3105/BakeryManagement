...
/**
 * 
 * 【S3-66 : SA710】 JWSリテラル検索（充填方法）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralZyutenSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralZyutenSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_jyutenhoho";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（充填方法）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
