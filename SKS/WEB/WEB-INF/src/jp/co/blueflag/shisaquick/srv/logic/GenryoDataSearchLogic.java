...
//add start --------------------------------------------------------------------------------------
//QP@00412_シサクイック改良 No.4
			strAllSql.append("	,CASE LEFT(tbl.cd_genryo,1) WHEN 'N' THEN '-' ELSE "
								+ "(CASE ISNULL(tbl.flg_shiyo,0) WHEN 1 THEN '○' ELSE '×' END) END flg_shiyo ");
			strAllSql.append("	,CASE LEFT(tbl.cd_genryo,1) WHEN 'N' THEN '' ELSE "
								+ "(CASE ISNULL(tbl.flg_mishiyo,0) WHEN 1 THEN '○' ELSE '' END) END flg_mishiyo ");
//add end --------------------------------------------------------------------------------------
...
//add start --------------------------------------------------------------------------------------
//QP@00412_シサクイック改良 No.4
			strSql.append(" ,ISNULL(M402.flg_shiyo,0) AS flg_shiyo");
			strSql.append(" ,ISNULL(M402.flg_mishiyo,0) AS flg_mishiyo");
//add end --------------------------------------------------------------------------------------
// ADD start 20121005 QP@20505 No.24
	...
	private void storageGenryouData(List<?> lstGenryouData, RequestResponsTableBean resTable)
		...
		try {
			...
			for (int i = 0; i < lstGenryouData.size(); i++) {
				...
				resTable.addFieldVale(i, "cd_busho", items[27].toString());
//add start --------------------------------------------------------------------------------------
//QP@00412_シサクイック改良 No.4
				resTable.addFieldVale(i, "flg_shiyo", toString(items[28]));
				resTable.addFieldVale(i, "flg_mishiyo", toString(items[29]));
//add end --------------------------------------------------------------------------------------
// ADD start 20121005 QP@20505 No.24
				resTable.addFieldVale(i, "ritu_msg", items[30].toString());
// ADD end 20121005 QP@20505 No.24
