...
/**
 * 
 * 【S3-70 : SA750】 JWSリテラル検索（単位）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralTaniSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralTaniSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_tani";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（単位）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
