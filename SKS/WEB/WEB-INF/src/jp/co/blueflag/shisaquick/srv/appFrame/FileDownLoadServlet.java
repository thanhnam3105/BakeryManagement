...
public class FileDownLoadServlet extends HttpServletBase {
	...
	public void doGet(HttpServletRequest HttpReq, HttpServletResponse HttpRes) {
		...
		//                                                        &DLPath=c:::d:::e
		//c:::d:::e　=　ダウンロードファイルの保存サブフォルダー名、
		//  c：ダウンロードパス（const値）、d：ファイルaのサブフォルダー、e：ファイルbのサブフォルダー
		//【QP@40404】add start TT E.Kitazawa 2014/09/08
//		BufferedReader br;
	...
	private void printPage(ArrayList<String> listFUrl, ArrayList<String> listDLPath){
		...
		try{
			...
			for (int i = 0; i < listFUrl.size(); i++){
				...
				out.println(listFUrl.get(i));
				out.println("</td><td>");
				out.println("<input type=\"button\" value=\"ダウンロード\" onclick=\"javascript:location.href='/" + strRootPath[strRootPath.length-1] + "/FileDownloadExec?FName=" + listFUrl.get(i) + "&DLPath=" + strDLPath +"';\">");
				out.println("</td></tr>");
