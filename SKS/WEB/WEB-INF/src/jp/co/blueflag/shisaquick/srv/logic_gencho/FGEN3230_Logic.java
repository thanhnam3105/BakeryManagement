 ...
 * @since  2014/09/11
 */
public class FGEN3230_Logic extends LogicBase{

	/**
	 * 職場マスタ検索
	 * : インスタンス生成
	 */
	public FGEN3230_Logic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			...
			// 検索結果がない場合
			if (lstRecset.size() == 0){
				em.ThrowException(ExceptionKind.警告Exception,"W000501", "職場マスタ", "", "");
			}
			...
			this.em.ThrowException(e, "職場マスタ検索処理が失敗しました。");
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning{
		...
		try {
			...
			// SQL文の作成
			strSql.append(" SELECT cd_shokuba  ");
			strSql.append("       ,nm_shokuba  ");
			strSql.append(" FROM  ma_shokuba  ");
			strSql.append(" WHERE  cd_kaisha = " + strKaishaCd );
			strSql.append("   AND  cd_seizokojo = '" + strCdSeizokojo + "'");
			strSql.append(" ORDER BY  cd_shokuba ");
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning{
		...
		try {
			for (int i = 0; i < lstMstData.size(); i++) {
				...
				// 結果をレスポンスデータに格納
				resTable.addFieldVale(i, "cd_shokuba", toString(items[0],""));
				resTable.addFieldVale(i, "nm_shokuba", toString(items[1],""));
			}
