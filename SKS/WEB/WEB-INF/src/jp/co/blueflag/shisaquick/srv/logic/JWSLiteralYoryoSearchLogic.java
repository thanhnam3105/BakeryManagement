...
/**
 * 
 * 【S3-69 : SA740】 JWSリテラル検索（容量）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralYoryoSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralYoryoSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_yokitani";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（容量）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
