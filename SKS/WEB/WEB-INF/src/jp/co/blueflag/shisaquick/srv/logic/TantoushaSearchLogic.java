...
public class TantoushaSearchLogic extends LogicBase{
	...
	private StringBuffer createSQL(RequestResponsKindBean reqData, StringBuffer strSql)
		...
		try {
			...
			//グループ選択時の会社コード取得
			kaishaCd = ((RequestResponsRowBean) ((RequestResponsTableBean) reqData.GetItem(0)).GetItem(0)).GetItemValue("cd_kaisha");
			if (kaishaCd == "") {
				kaishaCd = userInfoData.getCd_kaisha();		//自社
			}
...
//			strSql.append(" WHERE cd_group = " + groupCd);
			...
			// MOD 2015/03/30 TT.Kitazawa【QP@40812】No.24 start
			//  グループ選択時の会社コードを設定
//			strSql.append(" WHERE cd_kaisha = " + userInfoData.getCd_kaisha());
			strSql.append(" WHERE cd_kaisha = " + kaishaCd);
			// MOD 2015/03/30 TT.Kitazawa【QP@40812】No.24 end
			strSql.append(" AND   cd_group = " + groupCd);
	...
	private void storageTantoushaCmb(List<?> lstTantoushaCmb, RequestResponsTableBean resTable)
		...
		try {
			...
			for (int i = 0; i < lstTantoushaCmb.size(); i++) {
				...
				if (items[6] == null) {
					resTable.addFieldVale(i, "cd_yakushoku", "");
				} else {
