 ...
 * @since  2016/10/12
 */
public class FGEN3600_Logic extends LogicBase{
    ...
    throws ExceptionSystem, ExceptionUser, ExceptionWaning{
        ...
        try {
            ...
            // 有効開始日
            String dt_yuko = toString(reqData.getFieldVale(0, 0, "dt_yuko"));
            if(!dt_yuko.equals("")){
                if(strWork.length() > 0){
                    strWork.append(" AND ");
                }
                strWork.append(" LST.dt_yuko <= '" + dt_yuko + "' ");
            }
...
//            strSql.append("    ,LST.biko_kojo  ");									// 備考（工場）
            strSql.append("    ,LST.name_houzai  ");								// 版の包材名
    ...
    throws ExceptionSystem, ExceptionUser, ExceptionWaning{
        ...
        try {
        	...
        	for (int i = 0; i < lstGenkaHeader.size(); i++) {
                ...
                resTable.addFieldVale(i, "name_houzai", toString(items[41],""));	// 版の包材名
