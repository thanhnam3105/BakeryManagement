 ...
 * @since  2009/04/07
 */
public class CategoriSearchLogic extends LogicBase{

	/**
	 * コンボ用：カテゴリ情報検索DB処理用コンストラクタ
	 * : インスタンス生成
	 */
	public CategoriSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	private StringBuffer createSQL(RequestResponsKindBean reqData, StringBuffer strSql)
		...
		try {
			...
			if (gamenId.equals("60") || gamenId.equals("65")) {
				...
				if (kinoId.equals("10")) {	//CSV出力（一般）
					strSql.append(" WHERE flg_edit = 1");
					//ADD start 2014/10/10【QP@40404】
					strSql.append(" AND (flg_gencho <> 1 or flg_gencho is null) ");
					//ADD end 2014/10/10【QP@40404】

				} else if (kinoId.equals("20")) {	//編集（一般）
					strSql.append(" WHERE flg_edit = 1");
					//ADD start 2014/10/10【QP@40404】
					strSql.append(" AND (flg_gencho <> 1 or flg_gencho is null) ");
					//ADD end 2014/10/10【QP@40404】

				} else if (kinoId.equals("50")) {	//編集（システム管理者）
					//処理なし
					//ADD start 2014/10/10【QP@40404】
					strSql.append(" WHERE flg_gencho <> 1 or flg_gencho is null ");
					//ADD end 2014/10/10【QP@40404】
				}
			}
