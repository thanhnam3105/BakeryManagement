...
/**
 * 
 * 【S3-71 : SA760】 JWSリテラル検索（荷姿）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralNisugataSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralNisugataSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_nisugata";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（荷姿）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
