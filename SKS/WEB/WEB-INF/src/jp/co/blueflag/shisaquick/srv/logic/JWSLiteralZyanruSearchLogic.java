...
/**
 * 
 * 【S3-57 : SA620】 JWSリテラル検索（ジャンル）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralZyanruSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralZyanruSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_jyanru";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（ジャンル）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
