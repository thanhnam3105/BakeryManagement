...
//			storageGenryoData(lstRecset, resKind.getTableItem(0));
			storageGenryoData(lstRecset, resKind.getTableItem(0), blnKengen);
// 20160513  KPX@1600766 MOD end
	...
	public StringBuffer ｇenryoDataCreateSQL(RequestResponsKindBean reqData)
		...
		try {
			...
			strRetSQL.append("  ,ISNULL(convert(varchar(10),tbl.dt_kakunin,111),'') as dt_kakunin");
			strRetSQL.append("  ,tbl.tanka AS tanka ");
			strRetSQL.append("  ,tbl.budomari AS budomari ");

			strRetSQL.append("  ,tbl.id_toroku AS id_toroku ");
			strRetSQL.append("  ,convert(varchar(10),tbl.dt_toroku,111) AS dt_toroku ");
			...
			strRetSQL.append("  ,tbl.nm_kakunin AS nm_kakunin ");
//add start --------------------------------------------------------------------------------------
//QP@00412_シサクイック改良 No.4
			strRetSQL.append("  ,tbl.flg_shiyo AS flg_shiyo ");
			strRetSQL.append("  ,tbl.flg_mishiyo AS flg_mishiyo ");
//add end --------------------------------------------------------------------------------------
...
//				strRetSQL.append(" SELECT DISTINCT M401.cd_kaisha AS cd_kaisha ,M401.cd_genryo AS cd_genryo ");
	...
	private StringBuffer createSQL_select()
		...
		try {
			...
			strRetSQL.append("  ,ISNULL(convert(varchar(10),M401.dt_kakunin,111),'') as dt_kakunin");
			...
			//単価・歩留
			strRetSQL.append(" ,CASE WHEN M402.tanka IS NULL THEN ISNULL(CONVERT(VARCHAR,M402.tanka),'') ELSE CONVERT(VARCHAR,CONVERT(DECIMAL(10,2),M402.tanka)) END AS tanka ");
			...
			strRetSQL.append("  ,convert(varchar(10),M401.dt_toroku,111) AS dt_toroku ");
			...
			strRetSQL.append(" ,ISNULL(M1012.nm_user,'') as nm_kakunin");
//add start --------------------------------------------------------------------------------------
//QP@00412_シサクイック改良 No.4
...
//add end --------------------------------------------------------------------------------------
			...
			strRetSQL.append("      LEFT JOIN ma_user M1012");
			strRetSQL.append("      ON M1012.id_user = M401.id_kakunin");
...
//add start --------------------------------------------------------------------------------------
//QP@00412_シサクイック改良 No.4
			String strReqShiyoFlg = reqData.getFieldVale(0, 0, "flg_shiyo");
//add end --------------------------------------------------------------------------------------
			...
			//原料コード
			if ( strReqGenryoCd != "" ) {
				strRetSQL.append("  AND M401.cd_genryo LIKE '%" + strReqGenryoCd + "%'");
			...
			if ( strReqGenryoNm != "" ) {
				...
				//変換実行
				ArrayList arySearch = ImeSearch.ImmGetConvListChange(strReqGenryoNm);
				...
				for(int j = 0; j < arySearch.size(); j++){
					...
					//SQL設定
					strRetSQL.append(" M402.nm_genryo LIKE '%" + getStr + "%'");
			...
			if ( strReqTaishoGenryo1.equals("true") && strReqTaishoGenryo2.equals("false") ) {
				...
				if ( !strReqBushoCd.equals("zenkojo") ) {
					//部署コードが未入力でない場合
					if(strReqBushoCd != null && strReqBushoCd.length() > 0){
						strRetSQL.append("  AND ( M402.cd_busho =" + strReqBushoCd);
...
//QP@00412_シサクイック改良 No.4
			// 使用実績フラグ=1の場合、条件追加
			if (strReqShiyoFlg.equals("1")) {
				strRetSQL.append("  AND M402.flg_shiyo=1");
...
//	private void storageGenryoData(List<?> lstGenryoData, RequestResponsTableBean resTable)
	private void storageGenryoData(List<?> lstGenryoData, RequestResponsTableBean resTable, boolean kengen)
			throws ExceptionSystem, ExceptionUser, ExceptionWaning{
// 20160513  KPX@1600766 MOD end
...
// 20160513  KPX@1600766 MOD end
				...
				resTable.addFieldVale("rec" + i, "nm_kakunin", items[37].toString());
//add start --------------------------------------------------------------------------------------
//QP@00412_シサクイック改良 No.4
				resTable.addFieldVale("rec" + i, "flg_shiyo", items[38].toString());
				resTable.addFieldVale("rec" + i, "flg_mishiyo", items[39].toString());
//add end --------------------------------------------------------------------------------------
			}
