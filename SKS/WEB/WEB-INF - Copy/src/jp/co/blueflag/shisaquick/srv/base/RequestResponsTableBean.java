...
public class RequestResponsTableBean extends RequestResponsBean{
	 ...
	 * @throws ExceptionWaning
	 */
	public int getItemNo(String RecName , String FieldName)
	throws ExceptionSystem, ExceptionUser, ExceptionWaning{
		...
		try{
			//レコードIDの添字を取得
			recIndex = super.GetItemNo(RecName);
			
			//レコードIDが存在する場合
			if(recIndex > -1){
				
				//フィールドIDの添字を取得
				ret = getRowItem(RecName).GetItemNo(FieldName);
	 ...
	 * @throws ExceptionWaning
	 */
	public int getItemNo(int RecNo , String FieldName)
	throws ExceptionSystem, ExceptionUser, ExceptionWaning{
		...
		try{
				//フィールドIDの添字を取得
				ret = getRowItem(RecNo).GetItemNo(FieldName);
