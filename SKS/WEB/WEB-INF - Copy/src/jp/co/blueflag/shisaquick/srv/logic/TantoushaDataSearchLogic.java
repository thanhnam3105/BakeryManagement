...
public class TantoushaDataSearchLogic extends LogicBase{
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		//【QP@10713】2011/11/18 TT H.SHIMA ADD 営業判断用
		StringBuffer strSqlEigyo = new StringBuffer();
		List<?> lstRecsetEigyo = null;
		...
		try {
			...
			if (lstRecset.size() == 0){
				...
				//【QP@10713】2011/11/18 TT H.SHIMA ADD Start 営業判断用
				
				//SQL文の作成
				strSqlEigyo = genryoData2EigyoCreateSQL(reqData, strSqlEigyo);
				
				//営業検出するSQLを実行
				super.createSearchDB();
				lstRecsetEigyo = searchDB.dbSearch(strSqlEigyo.toString());
				
				//検索結果がある場合（営業担当者が該当）
				if(lstRecsetEigyo.size() > 0){
					//【QP@10713】2011/11/22 TT H.SHIMA メッセージ変更
//					em.ThrowException(ExceptionKind.警告Exception,"E000330", strSql.toString(), "", "");
					em.ThrowException(ExceptionKind.警告Exception,"E000331", strSql.toString(), "", "");
					
				//検索結果がない場合(営業も含め該当なし)
				}else if(lstRecsetEigyo.size() == 0){
					em.ThrowException(ExceptionKind.警告Exception,"W000401", strSql.toString(), "", "");
				}
				//【QP@10713】2011/11/18 TT H.SHIMA ADD End   営業判断用
			}
...
//				strWhere.append(strUserName + "'");
				strWhere.append(" M101.nm_user LIKE '%");
				strWhere.append(strUserName + "%'");
			}
...
//【QP@10713】2011/11/01 TT H.SHIMA -ADD Start
//営業は検索結果から除く
			if (strWhere.toString().equals("")) {
				strWhere.append(" WHERE");
			} else {
			...
			strWhere.append(" M1042.flg_eigyo IS NULL");
//【QP@10713】2011/11/01 TT H.SHIMA -ADD End
	 ...
	 * @throws ExceptionSystem 
	 */
	private StringBuffer genryoData2EigyoCreateSQL(RequestResponsKindBean reqData, StringBuffer strSql) 
		throws ExceptionSystem, ExceptionUser, ExceptionWaning{
...
//					resTable.addFieldVale(iRow-1, "nm_yakushoku", items[13].toString());
					resTable.addFieldVale(iRow-1, "cd_group", toString(items[8]));
					resTable.addFieldVale(iRow-1, "nm_group", toString(items[9]));
					resTable.addFieldVale(iRow-1, "cd_team", toString(items[10]));
					resTable.addFieldVale(iRow-1, "nm_team", toString(items[11]));
					resTable.addFieldVale(iRow-1, "cd_yakushoku", toString(items[12]));
					resTable.addFieldVale(iRow-1, "nm_yakushoku", toString(items[13]));
					//ここまで
