 ...
 *
 */
public class JWSLiteralShosuSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralShosuSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_shosu";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（少数指定）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
