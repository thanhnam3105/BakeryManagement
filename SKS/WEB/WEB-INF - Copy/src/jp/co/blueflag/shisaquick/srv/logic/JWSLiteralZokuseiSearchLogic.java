...
/**
 * 
 * 【S3-55 : SA600】 JWSリテラル検索（工程属性）ＤＢ処理の実装
 *  : JWSリテラル検索（工程属性）処理のDBに対する業務ロジックの実装
 * 
 * @author TT.katayama
 * @since 2009/04/07
 *
 */
public class JWSLiteralZokuseiSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralZokuseiSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_kote";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（工程属性）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
