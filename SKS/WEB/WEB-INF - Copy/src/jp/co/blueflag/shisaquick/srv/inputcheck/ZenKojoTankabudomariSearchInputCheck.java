...
public class ZenKojoTankabudomariSearchInputCheck extends InputCheck {
	...
	private void genryoSearchKeyCheck(RequestData checkData) throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			...
			//コードが入力されている場合
			if (!checkData.GetValueStr("SA790", 0, 0, "cd_genryo").equals("")) {
				// コードの入力桁数チェック（スーパークラスの入力桁数チェック）を行う。
				super.sizeCheckLen(checkData.GetValueStr("SA790", 0, 0, "cd_genryo"), "コード", 11);
			}
			//名前が入力されている場合
