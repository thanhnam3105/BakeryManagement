...
//				this.genryoUpdate(strNewCode);
			...
			} else if ( strReqShoriKbn.equals("shisan_rireki") ) {
				//処理区分：試算履歴時(shisan_rireki)

				//試算履歴用採番検索処理メソッドを呼出し、新規発行コードを取得する
				strNewCode = this.shisanRirekiSelect();
//				//新規発行コードを用いて、試算履歴用採番更新処理メソッドにて自動採番処理を行う。
//				this.shisanRirekiUpdate(strNewCode);
...
//			strSQL.append(" FROM ma_saiban M601 ");
	...
	/**
	 * 試算履歴順用採番検索処理
	 *  : 採番マスタより、試算履歴順用の新規発行コードを取得する。
	 * @return 新規発行コード
	 * @throws ExceptionSystem
	 * @throws ExceptionUser
	 * @throws ExceptionWaning
	 */
	private String shisanRirekiSelect()
		throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			...
			strSQL.append(" SELECT ISNULL(CONVERT(VARCHAR,MAX(M601.no_seq)+1),'') AS no_seq_max ");
//			strSQL.append(" FROM ma_saiban M601 ");
			strSQL.append(" FROM ma_saiban M601 WITH(UPDLOCK, ROWLOCK)");
			//採番マスタ.キー1 = 試算履歴順
			strSQL.append(" WHERE M601.key1='試算履歴順' ");
			...
			if ( lstSearchAry.size() >= 0 ) {
				...
				//MAX値を取得
				Object item = (Object)lstSearchAry.get(0);
				
				if ( !toString(item).isEmpty() ) {
					//新規発行コード　：　採番マスタ.採番の最大値+1
					strRetNewCode = lstSearchAry.get(0).toString();
			...
			//①：新規発行コードの値により、処理を振り分ける
			if ( strRetNewCode.equals("1") ) {
				
				//②：採番マスタ（ma_saiban）の登録処理を行う
				strUpdSQL.append(" INSERT INTO ma_saiban ");
				strUpdSQL.append("  (key1, key2, no_seq, id_toroku, dt_toroku, id_koshin, dt_koshin) ");
				strUpdSQL.append("  VALUES ( ");
				strUpdSQL.append("  '試算履歴順', '' ");							//key1, key2
				strUpdSQL.append("  ,1");												//新規発行コード
				strUpdSQL.append("  ," + strUserId + " ,GETDATE() ");		//更新者
				strUpdSQL.append("  ," + strUserId + " ,GETDATE() )");		//登録者
			
			} else {
				
				//③：採番マスタ（ma_saiban）.追番の更新処理を行う
				strUpdSQL.append(" UPDATE ma_saiban ");
				strUpdSQL.append("   SET no_seq =" + strRetNewCode);
				strUpdSQL.append("      ,id_koshin =" + strUserId);
				strUpdSQL.append("      ,dt_koshin = GETDATE() ");
				//採番マスタ.キー1 = 試算履歴順
				strUpdSQL.append(" WHERE key1='試算履歴順' ");
			...
			this.em.ThrowException(e, "試算履歴順用採番検索処理が失敗しました。");
...
//	/**
//	 * 試算履歴順用採番更新処理
//	 *  : 試算履歴順用の採番マスタデータの更新処理を行う。
//	 * @param strNewCode : 新規発行コード
//	 * @throws ExceptionSystem
//	 * @throws ExceptionUser
//	 * @throws ExceptionWaning
//	 */
//	private void shisanRirekiUpdate(String strNewCode)
//		throws ExceptionSystem, ExceptionUser, ExceptionWaning {
//		
//		StringBuffer strSQL = new StringBuffer();
//		
...
//			
//			//①：新規発行コードの値により、処理を振り分ける
//			if ( strNewCode.equals("1") ) {
//				
//				//②：採番マスタ（ma_saiban）の登録処理を行う
//				strSQL.append(" INSERT INTO ma_saiban ");
//				strSQL.append("  (key1, key2, no_seq, id_toroku, dt_toroku, id_koshin, dt_koshin) ");
//				strSQL.append("  VALUES ( ");
//				strSQL.append("  '試算履歴順', '' ");							//key1, key2
//				strSQL.append("  ,1");												//新規発行コード
//				strSQL.append("  ," + strUserId + " ,GETDATE() ");		//更新者
//				strSQL.append("  ," + strUserId + " ,GETDATE() )");		//登録者
//			
//			} else {
//				
//				//③：採番マスタ（ma_saiban）.追番の更新処理を行う
...
//				strSQL.append("      ,dt_koshin = GETDATE() ");
//				//採番マスタ.キー1 = 試算履歴順
//				strSQL.append(" WHERE key1='試算履歴順' ");
//				
//			}
//			
//			//③:作成したSQLを用いて、登録・更新処理を行う。
...
//			
//		} catch (Exception e) {
//			this.em.ThrowException(e, "試算履歴順用採番更新処理が失敗しました。");
//			
//		} finally {
//			//セッションのクローズ
//			if (execDB != null) {
