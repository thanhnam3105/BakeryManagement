...
/**
 * 
 * 【S3-72 : SA770】 JWSリテラル検索（取扱温度）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralOndoSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralOndoSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_toriatukaiondo";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（取扱温度）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
