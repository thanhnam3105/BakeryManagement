...
/**
 * 
 * 【S3-65 : SA700】 JWSリテラル検索（製造方法）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralSeizoSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralSeizoSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_seizohoho";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（製造方法）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
