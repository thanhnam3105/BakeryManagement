...
public class RGEN3050_inputcheck extends InputCheck {
    ...
    /**
     *  : コストテーブル登録・承認画面 登録ボタン押下時インプットチェック用コンストラクタ
     */
    ...
    private void costTableSearchCheck(RequestData checkData) throws ExceptionSystem, ExceptionUser, ExceptionWaning {
        ...
        try {
            ...
            // メーカー名
            super.hissuCodeCheck(checkData.GetValueStr("FGEN3050", 0, 0, "cd_maker"), "メーカー名");

            // ベース包材
            super.hissuCodeCheck(checkData.GetValueStr("FGEN3050", 0, 0, "cd_houzai"), "ベース包材名");

            // ベース版数・有効開始日
            super.hissuInputCheck(checkData.GetValueStr("FGEN3050", 0, 0, "no_basehansu"), "ベース版数・有効開始日");

            // コストテーブル版数
            super.hissuInputCheck(checkData.GetValueStr("FGEN3050", 0, 0, "no_hansu"), "コストテーブル版数");

            // 有効開始日
            super.hissuInputCheck(checkData.GetValueStr("FGEN3050", 0, 0, "dt_yuko"), "有効開始日");
            ...
            // 包材名 入力桁数チェック
            super.sizeCheckLen(toString(checkData.GetValueStr("FGEN3050", "cost_list", 0, "name_hansu"), "", ","),"コスト包材名",200);

            for(int i = 0; i < 30; i++) {

            	if (i <= 0) {
            		costTableInputCheck(checkData, i, "ロット数");
            	} else {
            		costTableInputCheck(checkData, i, "コスト");
            	}
    ...
    private void costTableInputCheck(RequestData checkData, int row, String name) throws ExceptionSystem, ExceptionUser, ExceptionWaning {
    	...
    	// 【QP@30297】 TT.nishigawa 課題対応 --------------------- start
        // 桁数チェック
    	...
    	// 【QP@30297】 TT.nishigawa 課題対応 --------------------- end
    }
}
