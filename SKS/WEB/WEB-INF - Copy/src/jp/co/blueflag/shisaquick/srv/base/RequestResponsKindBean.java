...
public class RequestResponsKindBean  extends RequestResponsBean {
	 ...
	 * @throws ExceptionWaning
	 */
	public int getItemNo(String TableName , String RecName , String FieldName)
	throws ExceptionSystem, ExceptionUser, ExceptionWaning{
		...
		try{
			//テーブルIDの添字を取得
			tblIndex = super.GetItemNo(TableName);
			
			//テーブルIDが存在する場合
			if(tblIndex > -1){
				
				//レコードIDの添字を取得
				ret = getTableItem(TableName).getItemNo(RecName, FieldName);
	 ...
	 * @throws ExceptionWaning
	 */
	public int getItemNo(String TableName , int RecNo , String FieldName)
	throws ExceptionSystem, ExceptionUser, ExceptionWaning{
		...
		try{
			...
			if(tblIndex > -1){
				...
				//レコードIDの添字を取得
				ret = getTableItem(TableName).getItemNo(RecNo, FieldName);
