 ...
 * @since  2014/01/25
 */
public class FGEN3020_Logic extends LogicBase{

	/**
	 * コストテーブル取得処理
	 * : インスタンス生成
	 */
	public FGEN3020_Logic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			...
			//データの検索（ベース単価）
//			this.getData(resKind, reqData);	//	【KPX@1602367】 add
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			...
			this.em.ThrowException(e, "コストテーブル一覧　データ検索処理が失敗しました。");
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning{
		...
		try {
			...
			String strChkKakunin = reqData.getFieldVale(0, 0, "chk_kakunin");	// 登録済み（確認）チェック
...
//			【KPX@1602367】 add end
			...
			strSql.append("    M1.nm_2nd_literal AS nm_base_houzai ,");
...
//			strSql.append("    LST.name_houzai AS nm_base_houzai, ");				//	【KPX@1602367】 add
			strSql.append("    LST.name_hansu AS nm_houzai ");				//	【KPX@1602367】 add
			strSql.append("FROM ");
...
//			strSql.append("    LEFT JOIN ma_user M3 ON M3.id_user = LST.id_toroku");
			...
			strSql.append(" ORDER BY cd_maker, cd_houzai, no_hansu desc ");	//	【KPX@1602367】 mod
...
//				resTable.addFieldVale(i, "nm_houzai", toString(items[4],""));		// 包材名
				resTable.addFieldVale(i, "nm_houzai", toString(items[9],""));		// 版の包材名:コスト包材名（旧:包材名）
				...
				resTable.addFieldVale(i, "nm_base_houzai", toString(items[4],""));	// ベース包材名
