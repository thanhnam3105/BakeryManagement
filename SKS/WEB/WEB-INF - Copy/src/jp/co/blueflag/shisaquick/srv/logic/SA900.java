 ...
 * 【SA900】 JWSリテラル検索（工程パターン）ＤＢ処理の実装
 * 
 * @author TT.Nishigawa
 * @since 2011/05/30
 *
 */
public class SA900 extends LogicBase {
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_kote_ptn";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（工程パターン）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
