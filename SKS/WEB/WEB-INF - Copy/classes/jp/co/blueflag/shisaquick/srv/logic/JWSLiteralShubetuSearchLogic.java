...
/**
 * 
 * 【S3-62 : SA670】 JWSリテラル検索（種別）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralShubetuSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralShubetuSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_syubetu";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（種別）ロジック管理処理に失敗しました。");
		} finally {
		}
		return resBean;
	}
