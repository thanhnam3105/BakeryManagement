...
/**
 * 
 * 【S3-56 : SA610】 JWSリテラル検索（一括表示）ＤＢ処理の実装
 ...
 *
 */
public class JWSLiteralIkatuSearchLogic extends LogicBase {
	 ...
	 * コンストラクタ
	 */
	public JWSLiteralIkatuSearchLogic() {
		//基底クラスのコンストラクタ
		super();
	...
	throws ExceptionSystem, ExceptionUser, ExceptionWaning {
		...
		try {
			//カテゴリコードの設定
			String strCd_Categori = "K_ikatuhyouzi";
			//検索処理を行い、結果を格納する。
			resBean = this.literalSearch.ExecLogic(reqData,strCd_Categori,userInfoData);
			
		} catch (Exception e) {
			this.em.ThrowException(e, "JWSリテラル検索（一括表示）ロジック管理処理に失敗しました。");
